...
def test_position_is_lagged():
    ...
    # 重建信号->仓位映射
    def score_to_pos(score, cfg):
        if pd.isna(score):
            return cfg.pos_low_risk
        if score >= 80:
            return cfg.pos_high_risk
        if score >= 65:
            return cfg.pos_mid_risk
        return cfg.pos_low_risk

    # 验证几个非第一行：position[i] 应来自 risk_score[i-1]
    for i in range(1, min(10, len(result))):
        prev_score = result["risk_score"].iloc[i - 1]
        expected_pos = score_to_pos(prev_score, cfg)
        ...
        assert abs(actual_pos - expected_pos) < 1e-9, (
            ...
            f"should yield pos={expected_pos}"
        )
